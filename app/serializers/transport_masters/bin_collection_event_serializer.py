...
from app.serializers.process.routeplan_serializer import RoutePlanSerializer
...
from app.serializers.user_creations.stafftemplate_serializer import StaffTemplateSerializer
...
class BinCollectionEventSerializer(TenancyReadSerializerMixin, serializers.ModelSerializer):
    ...
    def _resolve_routeplan(self, assignment):
        trip_definition = getattr(assignment, "trip_definition_id", None)
        return getattr(trip_definition, "routeplan_id", None)

    def _resolve_vehicle(self, assignment):
        routeplan = self._resolve_routeplan(assignment)
        return getattr(routeplan, "vehicle_id", None)
    ...
    def get_routeplan(self, obj):
        routeplan = self._resolve_routeplan(obj.trip_assignment_id)
        if not routeplan:
            return None
        return RoutePlanSerializer(routeplan, context=self.context).data
