...
class CompanyScopedViewSet(viewsets.ModelViewSet):
    ...
    def _company(self):
        ...
        payload = getattr(self.request, "jwt_payload", {}) or {}
        company_unique_id = payload.get("company_unique_id")
        if company_unique_id:
            return Company.objects.filter(unique_id=company_unique_id).first()
    ...
    def perform_create(self, serializer):
        ...
        if self._is_platform_super_admin():
            ...
            serializer.save(**save_kwargs,created_by=self.request.user)
            return
        else:
            company = self._company()
            if not company:
    ...
    def perform_update(self, serializer):
        ...
        serializer.save(**save_kwargs,updated_by=self.request.user)