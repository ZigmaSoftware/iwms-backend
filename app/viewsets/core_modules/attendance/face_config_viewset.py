...
Read once at app start and cached client-side; it is cheap. Requires
authentication like `register/`/`recognize/` do on this backend (see
`AUTH_ONLY_SUFFIXES` in `module_permission_middleware.py`) — it is registered
alongside them for the same reason, not because it exposes anything about any
individual.
"""
