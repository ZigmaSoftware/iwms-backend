...
Ported from the government app's `trip_lifecycle_viewset.py`. Before this, a
trip was started only as a *side effect* of the first bin scan
(`scan_bin_viewset._ensure_assignment_in_progress`), so a household trip was
never started at all — and ended only if every stop happened to resolve,
which a `Skipped` / collect-later stop makes impossible. Nothing auto-closed
trips, so those sat `In Progress` forever.
...
raises a `TripRetripRequest` (mandatory reason) for a supervisor to decide,
and the trip stays In Progress until they do — see
`app/services/retrip_service.py`.
"""
...
class TripLifecycleViewSet(viewsets.ViewSet):
    ...
    def _resolve_own_assignment(self, request, unique_id):
        ...
        Deliberately does NOT reuse `find_active_assignment_for_operator`
        (the home-page feed): that helper is about picking the CURRENT trip
        to scan against, but start/end need to resolve a Completed/Cancelled
        trip by id too, in order to return the accurate `ALREADY_ENDED` /
        `TRIP_CANCELLED` 409 below, instead of a generic 404 that reads like
        the trip was never the driver's.
        """
    ...
    def end(self, request, pk=None):
        try:
            operator, assignment = self._resolve_own_assignment(request, pk)
        except OperatorFlowError as exc:
            return Response({"code": exc.code, "detail": exc.message}, status=exc.http_status)
