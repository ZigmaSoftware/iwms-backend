...
from app.serializers.collections.panchayat_wise_collection_serializer import (
    PanchayatCollectionSerializer,
)
...
class PanchayatWiseCollectionViewSet(AuditViewSetMixin, CompanyScopedViewSet):
    ...
    def get_queryset(self):
        return (
            PanchayatCollection.objects.select_related(
                "collection_point_id",
    ...
    def list(self, request, *args, **kwargs):
        ...
        overall_total = queryset.aggregate(total=Sum("panchayat_total_weight"))

        return Response(
            {
                "daily_total_weight": daily_total["total"] or 0,
                "overall_total_weight": overall_total["total"] or 0,
                "panchayat_collections": self.get_serializer(queryset, many=True).data,
            }
        )
