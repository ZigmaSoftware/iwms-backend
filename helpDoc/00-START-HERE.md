# IWMS Backend — Help Docs (Start Here)

This `helpDoc/` folder explains the entire `iwms-backend` project from
scratch, assuming you know nothing about it yet — not the architecture, not
Django, not this team's specific workflow. Read the files in order the first
...
> module — masters, users, vehicles, schedules, complaints, reports — each
> exposed under its own URL group. Database tables are NOT created by
> pulling code; they appear only when someone runs `migrate` on that machine,
> and sample data appears only when someone runs `seed`.**

Everything else below explains why, and how.

## Reading order

1. **[01-architecture-overview.md](01-architecture-overview.md)** — What
   the project actually is: one Django project, one app, many URL groups.
   How a request travels from the browser to a database row and back.
...
3. **[03-app-structure.md](03-app-structure.md)** — A tour of `app/`:
   models, serializers, viewsets, permissions, middleware, services, and
   how the custom router turns a viewset into a URL.
4. **[04-commands-reference.md](04-commands-reference.md)** — Every command
   ...
   clear caches, plus the backfill commands.
...
6. **[06-gitignore-and-secrets.md](06-gitignore-and-secrets.md)** — What is
   ...
   per-machine scripts), and what to do about secrets already leaked.
7. **[07-deployment-and-troubleshooting.md](07-deployment-and-troubleshooting.md)** —
   First-time server setup, `ALLOWED_HOSTS`/CORS, static files, gunicorn,
   the cron jobs, and a troubleshooting table of real problems already hit.
...
│   ├── management/commands <- `seed` and the backfill commands
