...
class TestStateCreate:
    def test_basic_create(self, continent, country):
        s = State.objects.create(name="Kerala", label="KL", continent_id=continent.unique_id, country_id=country.unique_id)
        assert s.name == "Kerala"
        assert s.label == "KL"

    def test_unique_id_prefix(self, continent, country):
        s = State.objects.create(name="Goa", label="GA", continent_id=continent.unique_id, country_id=country.unique_id)
    ...
    def test_foreign_keys(self, state, continent, country):
        assert state.continent_id == continent.unique_id
        assert state.country_id == country.unique_id


@pytest.mark.django_db
class TestStateDefaults:
    def test_is_active_default_true(self, continent, country):
        s = State.objects.create(name="Assam", label="AS", continent_id=continent.unique_id, country_id=country.unique_id)
        assert s.is_active is True

    def test_is_deleted_default_false(self, continent, country):
        s = State.objects.create(name="Bihar", label="BR", continent_id=continent.unique_id, country_id=country.unique_id)
